...
async function updateHandler(req: NextRequest, admin: any, params: { params: { id: string } }) {
  try {
    ...
    const updateData: Partial<Review> = {
      ...
      pros: Array.isArray(body.pros) ? body.pros.filter((p: string) => p.trim()) : (review.toObject().pros || []),
      cons: Array.isArray(body.cons) ? body.cons.filter((c: string) => c.trim()) : (review.toObject().cons || []),
